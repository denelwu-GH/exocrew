...
ExoCrew 不是基础模型、Coding Agent Runtime、MCP Server 或 CI/CD 平台。它是一个以 6 个 Codex Skills 交付的**生产交付 Harness 层**：

1. `exocrew-delivery`：统筹范围、风险、证据与收口。
2. `product-brief`：把一句模糊需求变成用户、价值、边界和验收标准。
3. `engineering-guardrails`：守住架构、契约和单一真值。
4. `system-modernization`：治理迁移、重构、框架升级、等价验证、开源提炼、成熟度和生产替换。
5. `test-evidence`：让验证深度匹配风险，并拒绝假绿证据。
6. `safe-operations`：治理数据变更、迁移、发布、回滚和上线后验证。
...
- 每次新任务都要重新解释之前的决定；
- 一个人正在统筹交付，同时覆盖产品、研发、现代化、测试和运维。
- 旧系统正在跨语言迁移、技术现代化、生产替换或提炼成干净的通用版本。
