...
### 0 代码经验，也能立刻拥有把项目做出来、跑起来、安全上线的完整交付能力

## 不用先成为程序员，先给 Codex 装上一支不会乱来的研发团队。

**一个不懂代码的人，用 Codex，在约四个月里把一套复杂企业业务系统真正推到了生产。**

ExoCrew 是为 Codex 原生打包的 AI 软件交付外骨骼。它把这四个月踩过的坑、建立的门禁和跑通的交付方法，全部装进你的 AI 工作流。

**不是让小白一夜变成程序员，而是让你从第一天起，就能按产品、架构、研发、测试、安全和运维的完整方法，指挥 AI 交付真实项目。**
...
**[30 秒安装](#30-秒安装) · [从想法到上线](#从一句想法到真实上线小白也能掌握完整流程) · [看看装上了谁](#六个角色一条交付链) · [English](README.md)**
...
## 从一句想法到真实上线，小白也能掌握完整流程

你负责说清楚要解决的问题、做关键选择；ExoCrew 负责要求 AI 每一步都给出边界、证据和退路。

| 交付阶段 | ExoCrew 要求 AI 完成什么 | 你能得到什么 |
|---|---|---|
| **1. 软件任务合同** | 写清目标、非目标、不可退化项和验收标准 | 开工前就知道“做完”到底是什么 |
| **2. 只读勘察** | 先查架构、依赖、数据流和影响面，不急着改文件 | 避免 AI 没看懂项目就直接动手 |
| **3. 安全边界** | 保护 Secret、数据库、生产操作和外部写入 | 高风险动作不会被一句模糊指令触发 |
| **4. 最小实现** | 找准真相源、生成文件和迁移边界，分阶段修改 | 少改、改对，并且不破坏已有能力 |
| **5. 完整测试** | 覆盖单元、集成、E2E、正常、异常、边界和回归 | 不再把“命令跑绿”误认为“项目可用” |
| **6. 浏览器验收** | 检查控制台、网络、空态、错误态和移动端 | 真实页面能用，而不只是代码看起来正确 |
| **7. 子代理交叉审核** | 从架构、实现、测试、安全四条线独立复核 | 降低一个 AI 自己写、自己证明的盲区 |
| **8. Git 与发布** | 检查状态和差异，形成小提交、回滚点和发布候选 | 每次修改都能解释、能审查、能退回 |
| **9. 线上闭环** | 完成部署、冒烟、监控、告警、回滚和复盘 | 从“本地能跑”真正走到“线上可负责” |

**你不必立刻看懂每一行代码，但必须看懂四件事：要做什么、没有做什么、证据在哪里、出问题怎么回来。ExoCrew 让 AI 主动把这四个答案交给你。**

## 会让 AI 写代码，不等于会交付一个项目
...
**你缺的不是更多生成代码，而是一套让小白也能判断“现在到底能不能上线”的交付系统。**
...
### 0 代码经验，不等于只能做一个跑不起来的 Demo。

## 给 Codex 装上 ExoCrew，把你的下一句话变成一个可运行、可验证、可上线的项目。
